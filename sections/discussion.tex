...
learning in a regime close to the heuristic policy and far above the random
policy. This section steps back from the raw numbers and interrogates what
...
this reading. The heuristic used in the main comparison happens to sit
almost exactly at the reward peak of its threshold sweep, so
Table~\ref{tab:results} reports the comparison against the best possible
instance of a one-parameter rule. Under an adversarial or merely
off-peak instantiation of the same rule family, PPO already matches or
outperforms three of the five tested thresholds. In a production
setting where the optimal threshold is unknown and would itself have to
be discovered---potentially across multiple equipment classes or
contract types---the relative cost of learning the policy from
interactions becomes a much weaker disadvantage. The extensions listed
in Section~\ref{subsec:improvements} are designed precisely to move the
evaluation into regimes where this asymmetry reverses.
